...
export const CityBackground = memo(() => {
  ...
  const windowData = useMemo(() => {
    const seed = 42; // Fixed seed for consistent rendering
    const pseudoRandom = (n: number) => {
      const x = Math.sin(seed + n) * 10000;
      return x - Math.floor(x);
    };

    // Generate window states for each building
    return Array.from({ length: 200 }, (_, i) => ({
      lit: pseudoRandom(i) > 0.35,
      warm: pseudoRandom(i + 100) > 0.3, // warm yellow vs cool blue
      bright: pseudoRandom(i + 200) > 0.5,
    }));
  }, []);

  // Helper to generate windows for a building
  const generateWindows = (
    ...
    const cols = Math.floor((buildingWidth - paddingX * 2) / (windowWidth + gapX));
    const rows = Math.floor((buildingHeight - paddingY * 2) / (windowHeight + gapY));
    ...
    for (let row = 0; row < rows; row++) {
      for (let col = 0; col < cols; col++) {
        ...
        if (data.lit) {
          const wx = buildingX + paddingX + col * (windowWidth + gapX);
          const wy = buildingY + paddingY + row * (windowHeight + gapY);
          const color = data.warm ? 'hsl(45 80% 70%)' : 'hsl(200 70% 65%)';
          const opacity = data.bright ? 0.9 : 0.5;
  ...
  return (
    <div className="fixed inset-0 overflow-hidden focus-bg-transition">
      ...
      {/* Moon */}
      <div
        className="absolute top-[6%] left-[12%] w-14 h-14 rounded-full opacity-60"
        style={{
      ...
      <div className="absolute bottom-0 w-full h-72">
        <svg viewBox="0 0 1200 290" className="w-full h-full" preserveAspectRatio="xMidYMax slice">
          <defs>
            ...
            <linearGradient id="cityBuildingMid" x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stopColor="hsl(265 32% 17%)" />
              <stop offset="100%" stopColor="hsl(265 38% 12%)" />
            </linearGradient>
          ...
          {/* Front layer buildings (darker, closer) */}
